...
type Client = {
  ...
  parentA?: { fullName: string } | null;
  parentB?: { fullName: string } | null;
};
type Meeting = {
...
export default function Dashboard({
  ...
  return (
    <main className="dashboard-shell">
      ...
      {tab === "clients" && <section className="dashboard-card"><div className="section-heading"><div><p className="kicker">Directory</p><div className="heading-with-action"><h2>Clients</h2><details className="add-menu"><summary aria-label="Add client">+</summary><ClientForm families={families} /></details></div><p className="table-help">Dummy names are shown below so you can verify family and parent relationships.</p></div><input placeholder="Filter by name" value={clientFilter} onChange={(e) => setClientFilter(e.target.value)} /></div><table><thead><tr><th>Client</th><th>Type</th><th>Family account</th><th>Parents</th><th>Contact</th><th /></tr></thead><tbody>{filteredClients.map((client) => <tr key={client.id}><td><strong>{client.fullName}</strong><small className="muted-cell">{client.email || "No email"}</small></td><td><span className="type-pill">{statusLabel(client.clientType)}</span></td><td>{client.familyAccount?.accountName || "—"}</td><td>{client.parentA?.fullName || client.parentB?.fullName ? <span className="parent-list">A: {client.parentA?.fullName || "—"}<br />B: {client.parentB?.fullName || "—"}</span> : "—"}</td><td>{client.phoneNumber || client.email || "—"}</td><td className="row-actions"><details><summary>Edit</summary><form action={updateClient} className="edit-form"><input type="hidden" name="id" value={client.id} /><input name="fullName" defaultValue={client.fullName} required /><input name="email" defaultValue={client.email || ""} type="email" /><button className="primary-button">Save</button></form></details><form action={deleteClient}><input type="hidden" name="id" value={client.id} /><button className="danger">Delete</button></form></td></tr>)}</tbody></table></section>}
