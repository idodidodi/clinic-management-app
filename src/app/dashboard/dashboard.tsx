...
type Client = {
  ...
  invoiceMeetingName?: string | null;
  clientType: string;
...
type Meeting = {
  id: string; startsAt: Date | string; type: string; workflowStatus: string; tariff: string; invoiceMeetingName?: string | null;
...
export default function Dashboard({
  ...
  return (
    <main className="dashboard-shell">
      ...
      {clients.map((client) => <ClientViewModal key={`modal-${client.id}`} client={client} open={clientSettingsId === client.id} onClose={() => setClientSettingsId(null)} />)}
    </main>
  );
}
...
function ClientViewModal({ client, open, onClose }: { client: Client; open: boolean; onClose: () => void }) {
  ...
  const additionalFields: [string, string][] = [
    ...
    ["Invoice meeting name", client.invoiceMeetingName || "—"],
  ...
  return <div className={`client-modal-backdrop${open ? " client-modal-open" : ""}`} aria-hidden={!open}><div className="client-modal" ref={modalRef} role="dialog" aria-modal="true" aria-label={`${client.fullName} details`}>
    {!editing ? <><div className="client-modal-header"><div><p className="kicker">Client details</p><h2>{client.fullName}</h2></div><button type="button" className="modal-close" onClick={onClose} aria-label="Close">×</button></div><div className="client-fields">{(showMore ? [...basicFields, ...additionalFields] : basicFields).map(([label, value]) => <div className="client-field" key={label}><div><span>{label}</span><strong>{value}</strong></div><CopyButton value={value} /></div>)}</div><button type="button" className="secondary-button show-more-button" onClick={() => setShowMore((value) => !value)}>{showMore ? "Show less" : "Show more"}</button><div className="client-modal-actions"><button type="button" className="primary-button" onClick={() => setEditing(true)}>Edit</button><button type="button" className="danger" onClick={() => setConfirmDelete(true)}>Delete</button><button type="button" className="secondary-button" onClick={onClose}>Close</button></div>{confirmDelete && <div className="delete-confirmation"><p>Delete this client? This cannot be undone.</p><form action={deleteClient}><input type="hidden" name="id" value={client.id} /><button className="danger">Confirm delete</button><button type="button" className="secondary-button" onClick={() => setConfirmDelete(false)}>Cancel</button></form></div>}</> : <><div className="client-modal-header"><div><p className="kicker">Edit client</p><h2>{client.fullName}</h2></div><button type="button" className="modal-close" onClick={onClose} aria-label="Close">×</button></div><form action={updateClient} className="edit-form-modal"><input type="hidden" name="id" value={client.id} /><label>Full name<input name="fullName" defaultValue={client.fullName} required /></label>{(client.clientType === "CHILD" || client.clientType === "PARENT") && <label>Phone number<input name="phoneNumber" defaultValue={client.phoneNumber || ""} type="tel" placeholder="Phone number" /></label>}<label>Email<input name="email" defaultValue={client.email || ""} type="email" placeholder="Email address" /></label><label>Invoice name<input name="invoiceName" defaultValue={client.invoiceName || ""} /></label><label>Invoice meeting name<input name="invoiceMeetingName" defaultValue={client.invoiceMeetingName || ""} placeholder="Art therapy" /></label><label>Preferred contact<select name="preferredContact" defaultValue={client.preferredContact || ""}><option value="">Not specified</option><option value="EMAIL">Email</option><option value="WHATSAPP">WhatsApp</option></select></label><label>Comments<textarea name="comments" defaultValue={client.comments || ""} /></label><TariffInputs client={client} /><div className="edit-form-actions"><button className="primary-button">Save</button><button type="button" className="secondary-button" onClick={() => setEditing(false)}>Cancel</button></div></form></>}
  </div></div>;
}
