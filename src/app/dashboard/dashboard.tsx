...
import { createClient, createFamily, createMeeting, createPayment, deleteClient, deleteMeeting, deletePayment, updateClient, updateMeeting, updatePayment } from "./actions";
...
export default function Dashboard({
  ...
  return (
    <main className="dashboard-shell">
      ...
      {tab === "clients" && <section className="dashboard-card"><div className="section-heading"><div><p className="kicker">Directory</p><div className="heading-with-action"><h2>Clients</h2><details className="add-menu"><summary aria-label="Add client or family">+</summary><div className="add-panel"><h3>Create family account</h3><FamilyForm /><h3>Add client to family</h3><ClientForm families={families} /></div></details></div><p className="table-help">Synthetic names are used for development. Parent type can be Mom, Dad, or Other.</p></div><input placeholder="Filter by name" value={clientFilter} onChange={(e) => setClientFilter(e.target.value)} /></div><table><thead><tr><th>Client</th><th>Type</th><th>Family account</th><th>Parents</th><th>Contact</th><th /></tr></thead><tbody>{filteredClients.map((client) => <tr id={`client-${client.id}`} key={client.id}><td><strong>{client.fullName}</strong><small className="muted-cell">{client.email || "No email"}</small></td><td><span className="type-pill">{client.clientType === "PARENT" ? client.parentRole === "DAD" ? "Dad" : client.parentRole === "OTHER" ? "Other" : "Mom" : statusLabel(client.clientType)}</span></td><td>{client.familyAccount ? <a className="client-link" href={`#client-${client.id}`}>{client.familyAccount.accountName}</a> : "—"}</td><td>{client.parentA?.fullName || client.parentB?.fullName ? <span className="parent-list">{client.parentA ? <>{client.parentA.parentRole === "DAD" ? "Dad" : client.parentA.parentRole === "OTHER" ? "Other" : "Mom"}: <a className="client-link" href={`#client-${client.parentA.id}`}>{client.parentA.fullName}</a></> : "—"}<br />{client.parentB ? <>{client.parentB.parentRole === "DAD" ? "Dad" : client.parentB.parentRole === "OTHER" ? "Other" : "Mom"}: <a className="client-link" href={`#client-${client.parentB.id}`}>{client.parentB.fullName}</a></> : "—"}</span> : "—"}</td><td>{client.phoneNumber || client.email || "—"}</td><td className="row-actions"><details><summary>Edit</summary><form action={updateClient} className="edit-form"><input type="hidden" name="id" value={client.id} /><input name="fullName" defaultValue={client.fullName} required /><input name="email" defaultValue={client.email || ""} type="email" /><button className="primary-button">Save</button></form></details><form action={deleteClient}><input type="hidden" name="id" value={client.id} /><button className="danger">Delete</button></form></td></tr>)}</tbody></table></section>}
      {tab === "meetings" && <section className="dashboard-card"><div className="section-heading"><div><p className="kicker">Calendar</p><h2>Meetings</h2></div></div><MeetingForm families={families} /><MeetingTable meetings={meetings} onDelete={deleteMeeting} /></section>}
...
function ClientForm({ families }: { families: { id: string; accountName: string }[] }) {
  return <form className="inline-form client-add-form" action={createClient}><input name="fullName" required placeholder="Full name" /><select name="familyAccountId" required><option value="">Family account</option>{families.map((family) => <option key={family.id} value={family.id}>{family.accountName}</option>)}</select><select name="clientType" defaultValue="CHILD"><option value="CHILD">Child</option><option value="PARENT">Parent</option></select><select name="parentRole" defaultValue="OTHER"><option value="MOM">Mom</option><option value="DAD">Dad</option><option value="OTHER">Other</option></select><input name="email" type="email" placeholder="Email" /><button className="primary-button">Add client</button></form>;
}

function FamilyForm() {
  return <form className="compact-form" action={createFamily}><input name="accountName" required placeholder="Family account name" /><input name="invoiceName" required placeholder="Invoice name" /><p className="table-help">This creates one Mom and one Dad client automatically.</p><div className="role-row"><label>Parent 1<select name="parentOneRole" defaultValue="MOM"><option value="MOM">Mom</option><option value="DAD">Dad</option></select></label><label>Parent 2<select name="parentTwoRole" defaultValue="DAD"><option value="MOM">Mom</option><option value="DAD">Dad</option></select></label></div><button className="primary-button">Create family</button></form>;
}

function MeetingForm({ families }: { families: { id: string; accountName: string }[] }) {
  return <form className="inline-form" action={createMeeting}><select name="familyAccountId" required><option value="">Family account</option>{families.map((family) => <option key={family.id} value={family.id}>{family.accountName}</option>)}</select><select name="type" defaultValue="CHILD"><option value="CHILD">Child meeting</option><option value="PARENT_A">Mom meeting</option><option value="PARENT_B">Dad meeting</option><option value="BOTH_PARENTS">Mom + Dad meeting</option></select><input name="startsAt" type="datetime-local" required /><input name="tariff" type="number" min="0" step="0.01" placeholder="Tariff" required /><button className="primary-button">Add meeting</button></form>;
}
