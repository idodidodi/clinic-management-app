...
import { createClient, createMeeting, createPayment, deleteClient, deleteMeeting, deletePayment, updateClient, updateClinicTariffs, updateMeeting, updatePayment } from "./actions";
...
export default function Dashboard({
  clinicName, clients, families, meetings, payments, invoices, payers, defaultTariff, clinicTariffs,
}: {
...
function TariffInputs({ client }: { client?: Client }) {
  return <div className="tariff-inputs">{["CHILD", "PARENT_A", "PARENT_B", "BOTH_PARENTS"].map((type) => <label key={type}>{statusLabel(type)} tariff<input name={`tariff-${type}`} type="number" min="0" step="0.01" defaultValue={client?.tariffs?.find((tariff) => tariff.meetingType === type)?.tariff} placeholder="Clinic default" /></label>)}</div>;
}
