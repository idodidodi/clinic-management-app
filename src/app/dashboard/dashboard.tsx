...
function MeetingForm({ clients }: { clients: Client[] }) {
  ...
  return <form className="inline-form" action={createMeeting}><select name="type" value={type} onChange={(event) => setType(event.target.value)} required><option value="">Meeting type</option><option value="CHILD">Child meeting</option><option value="PARENT_A">Mom meeting</option><option value="PARENT_B">Dad meeting</option><option value="BOTH_PARENTS">Mom + Dad meeting</option></select><select name="clientId" required><option value="">Client</option>{eligible.map((client) => <option key={client.id} value={client.id}>{client.fullName}</option>)}</select><input name="invoiceMeetingName" placeholder="Invoice meeting name" /><input name="startsAt" type="date" defaultValue={new Date().toISOString().slice(0, 10)} /><button className="primary-button">Add meeting</button></form>;
}
