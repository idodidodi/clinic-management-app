...
export default function Dashboard({
  ...
  return (
    <main className="dashboard-shell">
      ...
      {tab === "clients" && <section className="dashboard-card"><div className="section-heading"><div><p className="kicker">Directory</p><div className="heading-with-action"><h2>Clients</h2><details className="add-menu"><summary aria-label="Add client or family">+</summary><div className="add-panel"><h3>Create parent client</h3><ClientForm /><h3>Add family member</h3><FamilyMemberForm clients={clients} /></div></details></div><p className="table-help">Start with a parent client, then add children, a second parent, or another family member.</p></div><input placeholder="Filter by name" value={clientFilter} onChange={(e) => setClientFilter(e.target.value)} /></div><table><thead><tr><th>Client</th><th>Type</th><th>Family members</th><th>Contact</th><th /></tr></thead><tbody>{filteredClients.map((client) => <tr id={`client-${client.id}`} key={client.id}><td><strong>{client.fullName}</strong><small className="muted-cell">{client.email || "No email"}</small></td><td><span className="type-pill">{client.clientType === "PARENT" ? client.parentRole === "DAD" ? "Dad" : client.parentRole === "OTHER" ? "Other" : "Mom" : client.clientType === "OTHER" ? "Other" : statusLabel(client.clientType)}</span></td><td><span className="parent-list">{[...(client.relationsFrom || []).map((relation) => ({ ...relation.relatedClient, relationType: relation.relationType })), ...(client.relationsTo || []).map((relation) => ({ ...relation.client, relationType: relation.relationType }))].map((member) => <span key={`${client.id}-${member.id}`}><span className="muted-cell">{member.relationType.toLowerCase()}:</span> <a className="client-link" href={`#client-${member.id}`}>{member.fullName}</a><br /></span>)}</span></td><td>{client.phoneNumber || client.email || "—"}</td><td className="row-actions"><details><summary>Edit</summary><form action={updateClient} className="edit-form"><input type="hidden" name="id" value={client.id} /><input name="fullName" defaultValue={client.fullName} required /><input name="email" defaultValue={client.email || ""} type="email" /><button className="primary-button">Save</button></form></details><form action={deleteClient}><input type="hidden" name="id" value={client.id} /><button className="danger">Delete</button></form></td></tr>)}</tbody></table></section>}
      {tab === "meetings" && <section className="dashboard-card"><div className="section-heading"><div><p className="kicker">Calendar</p><h2>Meetings</h2></div></div><MeetingForm clients={clients} /><MeetingTable meetings={meetings} onDelete={deleteMeeting} /></section>}
