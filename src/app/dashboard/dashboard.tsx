...
export default function Dashboard({
  ...
  return (
    <main className="dashboard-shell">
      ...
      {tab === "clients" && <section className="dashboard-card"><div className="section-heading"><div><p className="kicker">Directory</p><div className="heading-with-action"><h2>Clients</h2><details className="add-menu"><summary aria-label="Add client or family">+</summary><div className="add-panel"><h3>Create family account</h3><FamilyForm /><h3>Add client to family</h3><ClientForm families={families} /></div></details></div><p className="table-help">Mom and Dad labels are family-defined. Families can have two moms or two dads.</p></div><input placeholder="Filter by name" value={clientFilter} onChange={(e) => setClientFilter(e.target.value)} /></div><table><thead><tr><th>Client</th><th>Type</th><th>Family account</th><th>Parents</th><th>Contact</th><th /></tr></thead><tbody>{filteredClients.map((client) => <tr id={`client-${client.id}`} key={client.id}><td><strong>{client.fullName}</strong><small className="muted-cell">{client.email || "No email"}</small></td><td><span className="type-pill">{client.clientType === "PARENT" ? (client.parentRole === "DAD" ? "Dad" : "Mom") : statusLabel(client.clientType)}</span></td><td>{client.familyAccount ? <a className="client-link" href={`#client-${client.id}`}>{client.familyAccount.accountName}</a> : "—"}</td><td>{client.parentA?.fullName || client.parentB?.fullName ? <span className="parent-list">{client.parentA ? <>{client.parentA.parentRole === "DAD" ? "Dad" : "Mom"}: <a className="client-link" href={`#client-${client.parentA.id}`}>{client.parentA.fullName}</a></> : "—"}<br />{client.parentB ? <>{client.parentB.parentRole === "DAD" ? "Dad" : "Mom"}: <a className="client-link" href={`#client-${client.parentB.id}`}>{client.parentB.fullName}</a></> : "—"}</span> : "—"}</td><td>{client.phoneNumber || client.email || "—"}</td><td className="row-actions"><details><summary>Edit</summary><form action={updateClient} className="edit-form"><input type="hidden" name="id" value={client.id} /><input name="fullName" defaultValue={client.fullName} required /><input name="email" defaultValue={client.email || ""} type="email" /><button className="primary-button">Save</button></form></details><form action={deleteClient}><input type="hidden" name="id" value={client.id} /><button className="danger">Delete</button></form></td></tr>)}</tbody></table></section>}
